              ...
              <div>
                ...
                <div className="mt-1">
                  {bdmApprovals.length > 0 ? (
                    <StatusBadge status={bdmApprovals[0].status} variant={bdmApprovals[0].status === 'APPROVED' ? 'success' : bdmApprovals[0].status === 'REJECTED' ? 'danger' : 'warning'} />
                  ) : (
              ...
              <div>
                ...
                <div className="mt-1">
                  {clientVerifications.length > 0 ? (
                    <StatusBadge status={clientVerifications[0].status} variant={clientVerifications[0].status === 'CONFIRMED' ? 'success' : clientVerifications[0].status === 'REJECTED' ? 'danger' : 'warning'} />
                  ) : (
                    <span className="text-text-muted italic">Not Started</span>
                  )}
                </div>
              ...
              <div className="flex justify-between items-center mb-6 border-b border-border pb-4">
                ...
                {bdmApprovals.length > 0 && bdmApprovals[0].status === 'APPROVED' && 
                 ...
                 canCreateVerification && (
                  <Button variant="primary" size="sm" onClick={() => setIsVerificationModalOpen(true)}>
              ...
              {clientVerifications.length === 0 ? <p className="text-gray-500">No Client verifications found.</p> : (
                <ul className="space-y-4">
                  {clientVerifications.map(v => (
                    <li key={v.id} className="border p-4 rounded bg-gray-50">
                      <div className="flex justify-between">
                      ...
                      <p className="text-sm text-gray-600 mt-1">Verifier: {v.verifierName || 'Pending'}</p>
                    </li>
                  ))}
                </ul>
              )}
